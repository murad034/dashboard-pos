    ...
    public function uploadCSV(Request $request): JsonResponse
    {
        try {
            ...
            if (count($data) > 0) {
                ...
                $headers = ["locationemail", "locationname", "allocatedstaff", "status", "locationid", "address", "administrative_area_level_1", "cityLat", "cityLng", "country", "locality", "locationabn", "locationphone", "postal_code", "route", "street_number", "accountname", "accountnumber", "bankname", "bsb", "franchfee", "marketingfee"];
            }
                ...
                try {
                    ...
                    foreach ($file_data as $row) {
                        ...
                        foreach ($customer_data as $index => $val) {
                            $data[$index] = $row[$val];
                            if ($index == "locationemail") {
                                $email = strval($row[$val]);
                            } else if ($index == "locationid") {
                                $locationId = strval($row[$val]);
                            }
                        }
                        $update_data = array('$set' => $data);
                        $condition = array('locationemail' => $email, "locationid" => $locationId);
