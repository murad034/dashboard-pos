...
function initialize() {
    ...
    const stnm = document.getElementById('stnm');
    const stabn = document.getElementById('stabn');
    ...
    const timestmp = document.getElementById('timestmp');
    ...
    stnm.onclick = event => insertText(edit, '%storename%');

    stabn.onclick = event => insertText(edit, '%storeabn%');

    stphone.onclick = event => insertText(edit, '%storephone%');

    staddress.onclick = event => insertText(edit, '%storeaddress%');

    recnum.onclick = event => insertText(edit, '%recnum%');

    stfname.onclick = event => insertText(edit, '%staffname%');

    timestmp.onclick = event => insertText(edit, '%timestamp%');

    saleinfo.onclick = event => insertText(edit, `{border:space; width:*,5,10; text:nowrap; align:left}
|%items%
-
{width:*,20}
^TOTAL | ^%total%
